...
export function contactContentLengthAllowed(request: Request): boolean {
  const raw = request.headers.get("content-length");
  if (raw === null || raw === "") return true;
  if (!/^\d+$/.test(raw)) return false;
  return Number(raw) <= MAX_CONTACT_BODY_BYTES;
}

export async function readContactJsonBody(request: Request): Promise<unknown | undefined> {
